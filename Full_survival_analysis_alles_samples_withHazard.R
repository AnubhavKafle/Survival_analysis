...
expression_rna = expression1[,] #TO eliminate factor problems
...
match_index=which(rownames(clinical)%in% rownames(expression_rna)) #Check which patient ID are matching with the tumor data
...
for (genes in colnames(expression_rna_ordered))
{
  model_analysis = summary(coxph(Surv(clinical_survival$days_to_death,clinical_survival$event) ~ as.numeric(expression_rna_ordered[,genes]))) #P-value is 5th in the index of coefficient. Coxph model is used because of the continous event of gene expression
...
return(list(p_values.adjusted, gene_names, hazard_ratio)) #Return as a list to the function call assignment variable
